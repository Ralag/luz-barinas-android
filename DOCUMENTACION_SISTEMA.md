...
> **Propósito:** Este documento contiene TODA la información necesaria para entender, mantener y evolucionar el sistema PAC Barinas. Si cambias de IA o de desarrollador, este archivo es tu biblia.
...
8. [Sistema de Notificaciones y Alarmas](#8-sistema-de-notificaciones-y-alarmas)
...
| `parroquia` | TEXT | Parroquia del sector |
| `lastUpdatedMillis` | BIGINT | Timestamp última actualización |

> ⚠️ **IMPORTANTE:** La tabla `sectors` NO tiene columna `municipio` ni `block` ni `submittedAt`. Esto es diferente a `community_locations`.

#### `community_locations` — Sectores propuestos por la comunidad (pendientes de aprobación)
| Columna | Tipo | Descripción |
|---|---|---|
| `id` | TEXT PK | ID generado (`sec_community_xxx`) |
| `name` | TEXT NOT NULL | Nombre propuesto |
| `municipio` | TEXT NOT NULL | Municipio (ej: "Barinas") |
| `parroquia` | TEXT NOT NULL | Parroquia |
...
| `confirmedReportsCount` | INT DEFAULT 0 | Reportes |
| `withoutPowerPercentage` | INT DEFAULT 0 | % sin luz |
| `rotationBlock` | TEXT | Bloque de rotación |
| `submittedAt` | BIGINT | Timestamp de envío |

> ⚠️ **DIFERENCIAS CLAVE entre `sectors` y `community_locations`:**
> - `community_locations` TIENE `municipio`, `block`, `submittedAt`
> - `sectors` TIENE `lastUpdatedMillis`
> - Al aprobar un sector comunitario, se debe OMITIR `municipio` del upsert a `sectors`
...
| 4 | Comunitarios | `#tabCommunity` | Aprobación/rechazo de sectores propuestos por usuarios |
...
### Funciones Principales de `app.js` (63 funciones)
...
1. `PacAlarmScheduler.scheduleNextAlarm()` calcula el próximo corte para el bloque del usuario
2. Programa `AlarmManager.setExactAndAllowWhileIdle()` con X minutos de anticipación (configurable: 1-180 min)
3. Cuando suena la alarma → `PacAlarmReceiver` dispara `PacAlarmPlayer.startAlarm()`
4. Se reproduce un tono de alarma en loop con vibración
5. El usuario puede silenciar desde el FAB ("APAGAR ALARMA") o la notificación
6. Opcionalmente, se programa una segunda alarma de "restauración" cuando vuelve la luz

### Configuración del Usuario

- **Anticipación:** 1 a 180 minutos (default: 10 min)
- **Notificación push:** habilitada/deshabilitada
- **Alarma sonora:** habilitada/deshabilitada
- **Alarma de restauración:** habilitada/deshabilitada
- **Vibración:** habilitada/deshabilitada
